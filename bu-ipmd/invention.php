  ...
  <main >
    <div class="container">
      ...
      <section>
        <br><br>
        <center>
        <h1 class="h1 orange-text text-center mb-3"> INVENTION </h1>    
          <p class="black-text lead col-8">An invention consists of the creation of a shape, configuration or composition of pattern or color, or combination of pattern and color in three-dimensional form containing aesthetic value. An industrial design can be a two- or three-dimensional pattern used to produce a product, industrial commodity or handicraft.</p>
        </center>
        <!--Grid row-->
        <!-- Collapse buttons -->
        <div>
        ...
        <!--Modal Add Form-->
          <div class="modal fade" id="modalAddForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-lg cascading-modal modal-notify modal-danger" role="document">
                <form action="invention.php" method="post" enctype="multipart/form-data" class="mod">
                  <div class="modal-content">
                      <div class="modal-header text-center">
                          <h4 class="modal-title white-text w-100 font-weight-bold"> INVENTION ADD FORM</h4>
                      ...
                      <div class="modal-body mx-3">
                          <div class="md-form">
                              <input type="text" id="invention_application_no" name ="invention_application_no" class="form-control validate">
                              <label for="invention_application_no">Application Number</label>
                          </div>

                          <div class="md-form">
                              <textarea type="text" id="invention_title" name="invention_title" class="form-control md-textarea validate"></textarea>
                              <label for="invention_title">Title</label>
                          </div>
                          ...
                          <div class="md-form">
                              <textarea type="text" id="invention_inventors" name="invention_inventors" class="form-control md-textarea validate"></textarea>
                              <label for="invention_inventors">Inventor(s)</label>
                          </div>

                          <div class="row">
                            <div class="col">
                              <div class="md-form">
                                  <label for="invention_filing_date" disabled>Filling Date:</label><br>
                                  <input type="date" id="invention_filing_date" name="invention_filing_date" class="form-control validate">    
                              </div>
                            </div>
                            ...
                            <div class="col">
                              <div class="md-form">
                                  <label for="invention_issue_date" disabled>Issue Date:</label><br>
                                  <input type="date" id="invention_issue_date" name="invention_issue_date" class="form-control validate">    
                              </div>
                            </div>

                            <div class="col">  
                              <div class="md-form">
                                  <label for="invention_received_date" disabled>Received Date:</label><br>
                                  <input type="date" id="invention_received_date" name="invention_inventors" class="form-control validate">    
                              </div>
                            </div>

                          </div>
                            <div class="md-form">
                                <input type="number" id="invention_year" name="invention_year" maxlength="4" class="form-control validate" aria-describedby="um_year_helper" value="2018">
                                <label for="invention_year" disabled>Issue On (Year):</label><br>    
                            </div>
                            <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label getmdl-select getmdl-select__fix-height" style="width: 100%;">
        ...
        <div class="collapse" id="collapseExample">
          ...
          <div>
            ...
            <tbody>
              ...
              <?php
                   $fetch_cus = mysqli_query($connect, "SELECT * FROM invention ORDER BY invention_year DESC")
                   ...
                   while($row = mysqli_fetch_array($fetch_cus))
                   {
                    ...
                    <td class="text-center">'. $row['invention_year'].'</td>
                    <td class="text-center">'.$row['invention_application_no'].'</td>
                    <td class="text-center">'. $row['invention_title'].'</td>
                    <td class="text-center">'.$age.'</td>
                    <td class="text-center">'.$cat.'</td>
                    <td class="text-center">';
                    if(($row['invention_issue_date']=="")||($row['invention_issue_date']=="0000-00-00")){
                      echo "NOT YET ISSUED";
                    }
                    else{
                      echo $row['invention_issue_date'];
                    }
                    echo'</td> 
                    <td class="text-center">';
                    if(($row['invention_received_date']=="")||($row['invention_received_date']=="0000-00-00")){
                      echo "NOT YET PUBLISHED";
                    }
                    else{
                      echo $row['invention_received_date'];
                    }
                    echo'</td>
                    ...
                    <?php 
                      if(isset($_SESSION['user_name'])){
                        $rem_query = "SELECT max(in_remarks_id) AS max_value FROM invention_remarks WHERE invention_id=$invention_id";
                        $fetch_que =mysqli_query($connect,$rem_query);
                          $row = mysqli_fetch_assoc($fetch_que);
                          $rem_id=$row["max_value"];

                          $rem_query2 = "SELECT in_remarks_time,in_remarks_desc FROM invention_remarks WHERE in_remarks_id=$rem_id";
                          if($fetch_res = mysqli_query($connect, $rem_query2)){
                          $rem_row = mysqli_fetch_assoc($fetch_res);
                            echo'
                            
                            <td class="text-center">';
                            if(($rem_row['in_remarks_time'])==""){
                              
                            }

                            else{
                              echo'<p><small class="green-text">'.$rem_row['in_remarks_time'].'</small><small><br>'.$rem_row['in_remarks_desc'].'</small></p>
                            </td>';
                            }
                          }
                      ...
                      <?php
                      if(!isset($_SESSION['user_name'])){
                        echo '<td class="text-center"><a name="inventionview" href="functions/invention/view.php?invention_id='.$invention_id.'" target="_blank">
                        ...
                        echo'
                        <td class="text-center">
                          <a name="inventionedit" href="functions/invention/edit.php?invention_id='.$invention_id.'" target="_blank">
                          ...
                          <a name="inventionview" href="functions/invention/view.php?invention_id='.$invention_id.'" target="_blank">
                          <button class="mdl-button mdl-js-button mdl-button--icon mdl-js-ripple-effect mdl-button--colored" type="button" data-placement="top" title="View">
                            <i class="material-icons">remove_red_eye</i>
                          </button>
                          </button></a>
                      
                          <a name ="inventiondelete" class="delete" data-confirm="Are you sure to delete this item?" href="functions/invention/delete.php?invention_id='.$invention_id.'">
                          ...
                          <a name ="remarkview" href="functions/invention/remarks.php?invention_id='.$invention_id.'" target="_blank">
...
if (isset($_POST['insert_post'])) {
  ...
  $inventor = $_POST['invention_inventors'];
  ...
  $insert_invention = "insert into invention (invention_application_no,invention_title,agency_id,category_id,filestatus_id,invention_inventors,invention_filing_date,invention_issue_date,invention_received_date,invention_year)
  
  values('$invention_application_no','$invention_title','$agency_id','$category_id','$filestatus_id','$invention_inventors','$invention_filing_date','$invention_issue_date','$invention_received_date','$invention_year')";

   $insert_invent = mysqli_query($connect, $insert_invention);

   if ($insert_invent) {
    echo "<script>alert('Invention added!')</script>";
    echo "<script>window.open('invention.php?','_self')</script>";
   }
}
